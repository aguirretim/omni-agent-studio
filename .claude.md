...
## Active Goals
- [ ] Light mode polish: FolderBrowser.tsx macOS chrome is still dark — add a light-mode override for its toolbar/header (keep file rows dark is fine as a terminal feel)
- [ ] Light mode polish: add `color-scheme: dark` to SplitPanePreview wrapper so browser renders scrollbars correctly for its dark terminal look even in light mode
- [ ] Light mode polish: WorkspaceSelector recents dropdown needs light bg override (currently uses zinc-800 bg-zinc-800/90 backdrop which looks odd in light)
